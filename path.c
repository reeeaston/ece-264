...
bool checkPath(Maze* m, char* path) {
    ...
    while (path[pathPos] != '\0') {
        ...
        printf(
            "At pos %d. Maze pos: (%d, %d), Square type: %c, Next step: %c\n",
            pathPos, cur.xpos, cur.ypos, m->maze[cur.ypos][cur.xpos].type,
            path[pathPos]);
        ...
        case WEST:
            ...
            break;
        default:
            fprintf(stderr, "Malformed path string\n");
            exit(1);
        }
        ...
        printf("Moved to square: (%d, %d), Square type: %c\n", cur.xpos,
               cur.ypos, m->maze[cur.ypos][cur.xpos].type);
